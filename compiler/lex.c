...
#define warnp( p, ...) warnf(__FILE__ ":" STR(__LINE__), pos_string(&p), __VA_ARGS )
...
// skipping the spaces including comments
// Returns true if at least one spece is skipped
...
// Skips a block of code exculded from input by #if, #ifdef and the like
// valid tokens even if skipped.However, in reality , most compiler don't
...
// Reads a number literal.Lexer's grammer on numbers is not struct
...
static int read_escaped_char(){
    Pos p = get_pos(-1);
    int c = readc();
    // https://cm.bell-labs.com/who/ken/trust.html
    switch (c){
        case '\'': case '"': case '?' : case '\\':
            return c;
        case 'a': return '\a';
...
static void skip_block_comments(){
    Pos p = get_pos(-2);
...
// Reads a digraph starting with '%' .Diagraphs are alternative spellings
// for some punctuation charcters. They are useless in ASCII /
// implementing jut for standard compilance
...
static Token *do_read_token(){
    ...
    switch(c){
        ...
        case 'L': case 'U': {
            // Wide/char 32_t character/string literal
...
char *read_header_file_name(bool *std){
    ...
    while (!next(close)){
        int c = readc();
        if (c == EOF || c == '\n'){
            errorp(p, "permature end of header name");
        }
        buf_write(b, c);
    }
    if (buf_len(b) == 0){
