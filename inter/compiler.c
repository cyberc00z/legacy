...
struct compiler *compiler_new(){
	...
	scope.instructions->cap = 1024; // initial capacity of 1024 byte
...
/*   TODO: We probably want dynamic error message that includes parts of program string  */
...
size_t add_instruction(struct compiler *c, struct instruction *ins){
...
size_t add_constant(struct compiler *c, struct object *obj){
...
int compile_block_statement(struct compiler *compiler, struct block_statement *block){
...
int compile_statement(struct compiler *c, struct statement *stmt){
...
int compile_expression(struct compiler *c, struct expression *expr){
	...
	switch(expr->type){
		...
		case EXPR->INFIX:
			...
			switch (expr->infix.operator){
				...
				case OP_LT:
				        compiler_emit(c, OPCODE_LESSER_THAN);
				break;
	...
	case  EXPR_PREFIX: {
           ...
           case EXPR_IF: {
				 ...
				 size_t after_alternative_pos = c->scopes[c->scopes_index].instructions->size;
	 ...
	 case EXPR_STRING:{
         ...
         case EXPR_IDENT:{
				 ...
				 if (S != NULL){
					 ...
					 compiler_emit(c, s->scope== SCOPE_GLOBLA ? OPCODE_GLOBAL : OPCODE_GET_LOCAL, s->index);
				 } else {
        ...
        case EXPR_FUNCTION:
	      ...
	      case EXPR_CALL:{
				     ...
				     for (i< expr->call.arguments.size; i++){
					     ...
					     err=compile_expression(c, expr->call.arguments.values[]);
...
struct instruction *compiler_leave_scope(struct compiler *c){
	...
	// We transfer ownership of this scope's instructions to a compiler function
