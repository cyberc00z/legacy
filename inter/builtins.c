...
struct object *builtin_len(struct object_list *args){
	...
	if (args->size != 1){
		...
		return make_error_object("wrong number of arguments: excepted 1, got %d" , args->size);
	}

	struct object *arg = args->values[0];
	if (arg->type != OBJ_STRING){
	
	return make_error_object("argument to len () not supported : excepted %s, got %s", object_type_to_str(OBJ_STRING), object_type_to_str(arg->type));	
