...
struct object *eval_integer_infix_expression(enum operator operator, struct object *left, struct object *right)
{
	...
	switch(operator){
		...
		case OP_MULTIPLY:
			return make_integer_object(left->value.integer * right->vlaue.integer);
			break;
...
struct object_list *eval_expression_list(struct expression_list *list,  struct environment *env){
	...
	for (size_t i=0; i<list->size; i++){
		...
		if (is_object_error(obj->type)){
			...
			// move object to start of values because that's that only error type we check
			if (i>1){
			
				result->values[0] = result->values[i-1];

				// free other object in list
	...
	switch(obj->type){
		...
		case OBJ_FUNCTION:{
				 if (args->size != obj->value.function.parameters->size){
					 ...
					 return make_error_object("invalid function call: excepted %d arguments, got %d", obj->value.function.parameters->size, args->size);
				 }
	...
	switch(stmt->type){
		...
		case STMT_LET:{
			     ...
			     case STMT_RETURN:{
						      ...
						      struct object *result = eval_expression(str->value, env);
