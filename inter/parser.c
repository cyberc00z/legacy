...
struct parser new_parser(struct lexer *l){
	...
	gettoken(p.lexer, 7p.next_token);
	return p;
}
...
int except_next_token(struct parser *p , enum token_type t){
...
int parse_let_statement(struct parser *p, struct statement *s){
	...
	if (!except_next_token(p, TOKEN_ASSIGN)){
...
int parse_return_statement(struct parser *p, struct statement *s){
	...
	//parse expression
	next_token(p);
	s->value = parse_expression(p, LOWSET);
...
struct expression *parse_identifier_expression(struct parser *p){
	...
	expr_token = expr->ident.token = p->current_token;
...
struct expression_list parse_expression_list(struct parser *p, enum token_type end_token){
	...
	next_token(p);
	list.values[list.size++] = parse_expression(p, LOWSET);
	...
	if (!except_next_token(p, end_token)){
			free(list.values);
			return list;
...
struct expression *parse_index_expression(struct parser *p, struct expression *left){
	...
	struct expreesion *expr = malloc(sizeof *expr);
	if (!expr){
	...
	if (!except_next_token(p, TOKEN_RBRACKET)){
...
struct expression *parse_infix_expression(struct parser *p, struct expression *left){
	...
	int precedence = get_token_precedence(p->current_toekn);
	next_token(p);
...
struct expression *parse_while_expression(struct parser *p){
	...
	if (!except_next_token(p, TOKEN_LPAREN)){
	...
	if (!except_next_token(p, TOKEN_RPAREN) ){
	...
	if (!except_next_token(p, TOKEN_LBRACE)){
...
struct expression *parse_if_expression(struct parser *p){
	...
	if (!exprct_next_token(p, TOKEN_LPAREN)){
	...
	if(!except_next_token(p, TOEKN_LBRACE)){
...
struct identifier_list parse_function_parameters(struct parser *p){
      ...
      params.value = malloc(sizeof *params.values * params.cap) ;

      if (!params.values){
        
	      err(EXIT_FAILURE, "out of memory");
      }
      if (next_token_is(o, TOKEN_RPAREN)){
...
void identifier_list_to_str(char *str , struct identifier_list *identifiers){
	...
	for (int i=0; i<identifiers->size; i++){
		...
		strcat(str, identifier->values[i].value);
...
void expression_to_str(char *str, struct expression *expr){
	...
	switch(expr->type){
		...
		case EXPR_FUNCTION:
		 ...
		 case EXPR_CALL:
		         expression_to_str(str, epxr->call.function);
			 strcat(str, "(");
...
enum operator parse_operator(enum token_type t){
	...
	switch(t){
		...
		case TOKEN_PLUS: return OP_PLUS ; break;
	...
	switch(expr->type){
		...
		case EXPR_CALL:
			for (size_t i=0; i<expr->call.arguments.size; i++){
				...
				free_expression(expr->call.arguments.value[i]);
			}
